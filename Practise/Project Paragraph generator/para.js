...
const generate = () => {
    ...
    if (!num || num < 0 || num > 50) {
        para.innerHTML = `<P style='color:Red;text-align:center'><b>Please provide numerical value b/w 0 and 50</b></p>  <p>${getRandomText()}</p>`;

    }
    else if (num > texts.length) {
        let p = [];
        for (i = 0; i < num; i++) {
            p.push(getRandomText())
        }
        ...
        const data = texts.slice(0, num)

        shufflePassword(data);
...
const shufflePassword = (str) => str.sort(() => { Math.random() - 0.5 });
