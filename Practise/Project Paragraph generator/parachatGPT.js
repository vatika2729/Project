...
const itemInput = document.querySelector('#number');
...
const generate = () => {
    const num = Number(itemInput.value);

    if (!(num) || num < 0 || num > 50) {
        para.innerHTML = `<p style='color:Red;text-align:center'><b>Please provide a numerical value between 0 and 50</b></p><p>${getRandomText()}</p>`;
    } else {
        const selectedTexts = num > texts.length ? Array(num).fill(null).map(getRandomText) : texts.slice(0, num);
        if (num > texts.length) {
            shuffleArray(selectedTexts);
        }
        para.innerHTML = selectedTexts.map(text => `<p>${text}</p>`).join('');
    }
